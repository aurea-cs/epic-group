...
// `"0.48.0+dev.cjs"` is statically replaced with the build-specific
// version string in a Rollup build, and a consumer's bundler `define` can
// inject it the same way — so the exact `"0.48.0+dev.cjs"` member
...
try {
  envLexicalVersion = "0.48.0+dev.cjs";
} catch (_unused) {
  ...
  insertText(text) {
    ...
    if (this.isCollapsed() && startOffset === firstNodeTextLength && ($isTokenOrSegmented(firstNode) || !firstNode.canInsertTextAfter() || !firstNodeParent.canInsertTextAfter() && firstNode.getNextSibling() === null)) {
      ...
      if ($getCompositionKey() !== null) {
        ...
        firstNode = firstNode.setMode('normal').setFormat(format).setStyle(style);
      } else {
        ...
        if ($isTextNode(lastNode) && !$isTokenOrTab(lastNode) && endOffset !== lastNode.getTextContentSize()) {
          ...
          if (!$isRootNode(endPoint.getNode()) && endPoint.type === 'text') {
            if (!$isTextNode(lastNode)) {
              formatDevErrorMessage(`insertText: lastNode is not a TextNode`);
            }
