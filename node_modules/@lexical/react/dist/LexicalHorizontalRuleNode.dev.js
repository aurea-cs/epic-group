...
function HorizontalRuleComponent({
  ...
  react.useEffect(() => {
    return lexical.mergeRegister(editor.registerCommand(lexical.CLICK_COMMAND, event => {
      ...
      if (lexical.getComposedEventTarget(event) === hrElem) {
        if (!event.shiftKey) {
          clearSelection();
        }
  ...
  react.useEffect(() => {
    ...
    if (hrElem !== null) {
      if (isSelected) {
        lexical.addClassNamesToElement(hrElem, isSelectedClassName);
      } else {
        lexical.removeClassNamesFromElement(hrElem, isSelectedClassName);
      }
    }
