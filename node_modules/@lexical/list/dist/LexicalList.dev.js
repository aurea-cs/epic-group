...
function $handleListItemBackspaceAdjacentToDecorator() {
  ...
  const anchorNode = selection.anchor.getNode();
  const listItem = lexical.$findMatchingParent(anchorNode, $isListItemNode);
  if (!$isListItemNode(listItem)) {
    return false;
  }
  ...
  if (!lexical.$isDecoratorNode(previousBlock) || previousBlock.isIsolated() || !(previousBlock.isKeyboardSelectable() || !previousBlock.isInline())) {
    return false;
  }
  // Demote the first list item to a paragraph and slot it in before the list.
  const paragraph = lexical.$createParagraphNode().append(...listItem.getChildren());
