...
const HeadingRule = /* @__PURE__ */html.defineImportRule({
  $import: (ctx, el) => {
...
const QuoteRule = /* @__PURE__ */html.defineImportRule({
  $import: (ctx, el) => {
 ...
 * @experimental
 */
const ShadowRootQuoteRule = /* @__PURE__ */html.defineImportRule({
  $import: (ctx, el) => {
    const node = $createQuoteNode({
      shadowRoot: true
    });
 ...
 */
const GoogleDocsTitleParagraphRule = /* @__PURE__ */html.defineImportRule({
  $import: (ctx, el, $next) => {
...
const GoogleDocsTitleSpanRule = /* @__PURE__ */html.defineImportRule({
...
const RichTextExtension = /* @__PURE__ */lexical.defineExtension({
 ...
 */
const RichTextImportExtension = /* @__PURE__ */lexical.defineExtension({
...
function $tryDecoratorLineNavigation(selection, isBackward) {
  ...
  if (focus.type === 'element' && lexical.$isElementNode(focusNode) && (lexical.$isRootNode(focusNode) || lexical.$isShadowRootNode(focusNode))) {
...
function registerRichText(editor, escapeFormatTriggers = extension.signal(DEFAULT_ESCAPE_FORMAT_TRIGGERS)) {
  const removeListener = lexical.mergeRegister(editor.registerCommand(lexical.CLICK_COMMAND, () => {
    ...
    if (lexical.$isNodeSelection(selection$1)) {
      ...
      if (nodes.length > 0) {
        event.preventDefault();
        $exitNodeSelectionToward(nodes[0], selection.$isParentRTL(nodes[0]) ? 'next' : 'previous');
        return true;
      }
    }
    ...
    if (!event.shiftKey && $tryBlockCursorShadowRootNavigation(selection$1, selection.$isParentRTL(selection$1.anchor.getNode()) ? 'next' : 'previous')) {
      event.preventDefault();
      return true;
    }
    if (!event.shiftKey) {
    ...
    if (lexical.$isNodeSelection(selection$1)) {
      ...
      if (nodes.length > 0) {
        event.preventDefault();
        $exitNodeSelectionToward(nodes[0], selection.$isParentRTL(nodes[0]) ? 'previous' : 'next');
        return true;
      }
    }
    if (!lexical.$isRangeSelection(selection$1)) {
    ...
    if (!event.shiftKey && $tryBlockCursorShadowRootNavigation(selection$1, selection.$isParentRTL(selection$1.anchor.getNode()) ? 'previous' : 'next')) {
      event.preventDefault();
      return true;
    }
    if (!event.shiftKey) {
