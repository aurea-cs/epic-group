...
import { $insertDataTransferForRichText, caretFromPoint, $handleRichTextDrop, setLexicalClipboardDataTransfer, $getClipboardDataFromSelection, $writeDragSourceToDataTransfer, copyToClipboard } from '@lexical/clipboard';
import { NormalizeInlineElementsExtension, NormalizeTripleClickSelectionExtension, effect, namedSignals, signal } from '@lexical/extension';
import { $isParentRTL, $shouldOverrideDefaultCharacterSelection, $moveCharacter } from '@lexical/selection';
import { $handleIndentAndOutdent, eventFiles, objectKlassEquals, $getNearestBlockElementAncestorOrThrow } from '@lexical/utils';
export { eventFiles } from '@lexical/utils';
import { setNodeIndentFromDOM, $setFormatFromDOM, $setDirectionFromDOM, isHTMLElement, defineExtension, configExtension, safeCast, shallowMergeConfig, $applyNodeReplacement, createCommand, ElementNode, $getDocument, addClassNamesToElement, $createParagraphNode, $getState, $setState, createState, mergeRegister, CLICK_COMMAND, $getSelection, $isNodeSelection, $isRangeSelection, COMMAND_PRIORITY_EDITOR, DELETE_CHARACTER_COMMAND, DELETE_WORD_COMMAND, DELETE_LINE_COMMAND, CONTROLLED_TEXT_INSERTION_COMMAND, REMOVE_TEXT_COMMAND, FORMAT_TEXT_COMMAND, $formatText, SET_TEXT_FORMAT_COMMAND, $setTextFormat, FORMAT_ELEMENT_COMMAND, $findMatchingParent, $isElementNode, INSERT_LINE_BREAK_COMMAND, INSERT_PARAGRAPH_COMMAND, INSERT_TAB_COMMAND, $createTabNode, $insertNodes, INDENT_CONTENT_COMMAND, OUTDENT_CONTENT_COMMAND, KEY_ARROW_UP_COMMAND, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_LEFT_COMMAND, KEY_ARROW_RIGHT_COMMAND, KEY_BACKSPACE_COMMAND, IS_IOS, CAN_USE_BEFORE_INPUT, KEY_DELETE_COMMAND, KEY_ENTER_COMMAND, $isDecoratorNode, IS_SAFARI, IS_APPLE_WEBKIT, KEY_ESCAPE_COMMAND, DROP_COMMAND, $getNearestNodeFromDOMNode, $createRangeSelection, $isTextNode, $normalizeSelection__EXPERIMENTAL, $setSelection, DRAGSTART_COMMAND, DRAGOVER_COMMAND, SELECT_ALL_COMMAND, $selectAll, $getSlotFrame, COPY_COMMAND, CUT_COMMAND, PASTE_COMMAND, isDOMNode, $isSelectionCapturedInDecoratorInput, KEY_SPACE_COMMAND, KEY_TAB_COMMAND, MOVE_TO_END, MOVE_TO_START, $getSiblingCaret, $setSelectionFromCaretRange, $getCollapsedCaretRange, $caretFromPoint, $isRootNode, $isShadowRootNode, $isRootOrShadowRoot, $getEditor, getDOMSelection, $hasAncestor, $getRoot, CUT_TAG, PASTE_TAG, $comparePointCaretNext, $getCaretRange, $isSiblingCaret, $extendCaretToRange, $needsBlockCursorBeside, $normalizeCaret, $getChildCaret, $createNodeSelection } from 'lexical';
import { DragonExtension } from '@lexical/dragon';
import { defineImportRule, sel, BlockSchema, CoreImportExtension, DOMImportExtension } from '@lexical/html';
...
const RichTextExtension = /* @__PURE__ */defineExtension({
  build: (_editor, config) => namedSignals(config),
  config: /* @__PURE__ */safeCast(DEFAULT_RICH_TEXT_CONFIG),
 ...
 */
const RichTextImportExtension = /* @__PURE__ */defineExtension({
 ...
 */
const quoteShadowRootState = /* @__PURE__ */createState('shadowRoot', {
  parse: Boolean
});
...
function $escapeFormatsForTrigger(selection, trigger, direction, config) {
  ...
  for (const [formatKey, triggers] of Object.entries(config)) {
    ...
    if (triggers.onlyAtBoundary) {
      if (!isBoundary || !anchorNode || !$isTextNode(anchorNode) || !anchorNode.hasFormat(format)) {
        continue;
      }
...
function $tryDecoratorLineNavigation(selection, isBackward) {
  ...
  if (focus.type === 'element' && $isElementNode(focusNode) && ($isRootNode(focusNode) || $isShadowRootNode(focusNode))) {
  ...
  const stillInSameBlock = movedNode.is(topBlock) || $hasAncestor(movedNode, topBlock);
  if (stillInSameBlock) {
    return false;
  }
  $selectNode(adjacentSibling.__key);
...
function $tryInlineGridLineNavigation(selection, isBackward) {
  ...
  const direction = isBackward ? 'previous' : 'next';
  const siblingBlock = $getSiblingCaret(parentBlock, direction).getNodeAtCaret();
...
function $exitNodeSelectionToward(node, direction) {
  ...
  if (sibling !== null && $isElementNode(sibling.origin) && !sibling.origin.isInline() && sibling.origin.isShadowRoot()) {
...
function $promoteNodeSelectionToBlockEdge(selection, isBackward, event) {
  ...
  const sorted = nodes.map(node => $getSiblingCaret(node, 'next')).sort($comparePointCaretNext);
  const targetNode = (isBackward ? sorted[0] : sorted[sorted.length - 1]).origin;
  const block = $findMatchingParent(targetNode, n => n !== targetNode && $isElementNode(n) && !n.isInline()) ?? $getRoot();
...
function registerRichText(editor, escapeFormatTriggers = signal(DEFAULT_ESCAPE_FORMAT_TRIGGERS)) {
  const removeListener = mergeRegister(editor.registerCommand(CLICK_COMMAND, () => {
    const selection = $getSelection();
    ...
    if (!event.shiftKey) {
      $escapeFormatsForTrigger(selection, 'arrow', 'start', escapeFormatTriggers.peek());
    }
    ...
    if (!event.shiftKey) {
      $escapeFormatsForTrigger(selection, 'arrow', 'end', escapeFormatTriggers.peek());
    }
    ...
    if ($isNodeSelection(selection)) {
      const nodes = selection.getNodes();
      if (nodes.length === 1 && $isDecoratorNode(nodes[0]) && !nodes[0].isInline()) {
    ...
    const anchorBlock = $findMatchingParent(anchor.getNode(), node => $isElementNode(node) && !node.isInline());
...
export { $createHeadingNode, $createQuoteNode, $isHeadingNode, $isQuoteNode, DRAG_DROP_PASTE, HeadingNode, QuoteNode, RichTextExtension, RichTextImportExtension, RichTextImportRules, ShadowRootQuoteRule, quoteShadowRootState, registerRichText };
