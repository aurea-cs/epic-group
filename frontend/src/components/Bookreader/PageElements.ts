...
export const submitPageResponses = async (
    itemId: string,
    pageNumber: number,
    studentId: string,
    responses: Record<string, any>
): Promise<{ results: Array<{ element_id: string; is_correct: boolean | null; response: any }> }> => {
    const res = await fetch(`${API_URL}/api/modules/items/${itemId}/pages/${pageNumber}/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
