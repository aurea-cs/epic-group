...
const ProfessorsAdminScreen: React.FC<ProfessorsAdminScreenProps> = ({ user }) => {
  ...
  useEffect(() => { fetchProfessors(); fetchCenters() }, [fetchProfessors, fetchCenters])
  ...
  return (
    <>
      {showAddModal && <AddProfessorModal onClose={() => setShowAddModal(false)} onSuccess={fetchProfessors} />}
          ...
          {loading ? (
            ...
            <div className="professors-admin-table-scroll">
              <table className="professors-admin-table">
                ...
                <tbody>
                  {filtered.map((professor, idx) => (
                    <tr
                      key={professor.id}
                      onClick={() => navigate(`/profesors/${professor.id}`)}
                    ...
                    >
                      ...
                      <td style={tdStyle}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                          <div style={{ width: '38px', height: '38px', borderRadius: '50%', flexShrink: 0, background: `linear-gradient(135deg, ${avatarColor(professor.name)})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.95rem', fontWeight: 700, color: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
                            {professor.avatar_url
                              ? <img src={professor.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      ...
                      {/* Email */}
                      <td style={tdStyle}>
                        <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.87rem' }}>{professor.email}</span>
                      </td>
...
export default ProfessorsAdminScreen
